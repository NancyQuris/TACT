'''
    implementation from https://github.com/fiveai/LAME, https://github.com/mr-eggplant/FOA/blob/main/tta_library/lame.py
'''
...
class LAME(nn.Module):
    ...
    """  
    def __init__(self, model, num_classes, hparams, projection_fn=None):
        """
        Args:
            cfg (CfgNode):
        """
        ...
        self.force_symmetry = hparams['force_symmetry']
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            if self.projection_fn is not None:
                out = self.projection_fn(out, x.device)
