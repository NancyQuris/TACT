'''
    from https://github.com/mr-eggplant/FOA
'''
...
class FOA(nn.Module):
    ...
    def __init__(self, model, num_classes, hparams):
        ...
        self.fitness_lambda = hparams['fitness_lambda']

        self.model = PromptTransformer(model, hparams['num_prompts'], self.hparams['args']['dataset'])
        ...
        if self.hparams['args']['dataset'] != 'imagenet_r':
            self.mask = None
        else:
            ...
            self.mask = [wnid in imagenet_r_wnids for wnid in all_wnids]

    def _init_cma(self):
        """CMA-ES initialization"""
        dim = self.model.prompts.numel()
        popsize = int(4 + self.hparams['num_prompts']*np.log(dim)) 
        cma_opts = {
            'seed': 2020,
            'popsize': popsize,
            'maxiter': -1,
    ...
    def _get_shift_vector(self):
        ...
        if self.hist_stat is None:
            ...
            return self.train_info[1][-768:].cuda() - self.hist_stat
    ...
    def forward(self, x):
        ...
        for j, prompt in enumerate(prompts):
            ...
            outputs, loss, batch_mean = forward_and_get_loss(x, self.model, self.fitness_lambda, self.train_info, shift_vector, self.mask)
    ...
    def obtain_origin_stat(self, train_loader):
        ...
        with torch.no_grad():
            for _, dl in enumerate(train_loader):
                images = dl[0].cuda()
                feature = self.model.layers_cls_features(images).cpu()
...
def forward_and_get_loss(images, model, fitness_lambda, train_info, shift_vector, imagenet_mask=None):
    ...
    output = model.vit.classifier(cls_features)
    ...
    if shift_vector is not None:
        output = model.vit.classifier(cls_features + 1. * shift_vector)
        if imagenet_mask is not None:
            output = output[:, imagenet_mask]
...
class PromptTransformer(nn.Module):
    '''
        Transformer with added prompts at the input layer, 
        adjusted for our defined model
    '''
    def __init__(self,
                transformer,
                num_prompts,
        ...
        self.prompt_dim = transformer.n_output
        self.dataset = dataset

        if self.dataset.startswith('imagenet'):
            self.backbone = self.vit.featurizer.model
        else:
            self.backbone = self.vit.featurizer

        if num_prompts > 0:
            self.prompts = nn.Parameter(torch.zeros(1, num_prompts, self.prompt_dim)).cuda()
            val = math.sqrt(6. / float(3 * reduce(mul, (self.backbone.patch_size, self.backbone.patch_size), 1) + self.prompt_dim))
    ...
    def _collect_layers_features(self, x):
        ...
        for i in range(len(self.backbone.encoder.layers)):
            x = self.backbone.encoder.layers[i](x)
            # get the statistics of the features after normalization 
            if i < len(self.backbone.encoder.layers) - 1:
                cls_features.append(self.backbone.encoder.layers[i+1].ln_1(x[:, 0]))
            else:
                cls_features.append(self.backbone.encoder.ln(x[:, 0]))
